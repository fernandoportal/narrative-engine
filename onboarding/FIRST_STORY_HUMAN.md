# First Story Onboarding (Human)

Use this guide directly in Antigravity. Keep it simple and move fast.

## Step 1 - Story idea (1 sentence)

Open `story/logline.md`.

Write one sentence using this format:
`When [event], a [protagonist] must [goal] before [deadline], or [consequence].`

## Step 2 - Protagonist

Open `characters/protagonist.md`.

Fill these fields:

- Name
- Age
- Key trait
- Problem they have
- Want
- Need

Keep each line short.

## Step 3 - Simple structure

Open `structure/beat-sheet.md`.

Write only these beats:

- Opening: where the story starts
- Midpoint: what changes everything
- Ending: final outcome

Do not over-plan. Just lock the spine.

## Step 4 - First scene

Open `screenplay/selected-scenes/opening.fountain`.

Write one short scene:

- scene heading (INT./EXT. + place + time)
- 3 to 6 action lines
- brief dialogue exchange
- clear tension

## Step 5 - Save version

Ask your AI assistant: `save my narrative changes`

It should classify files, branch if needed, commit, and push.

## Quick check before you stop

- [ ] `story/logline.md` has one clear sentence
- [ ] `characters/protagonist.md` has core identity and conflict
- [ ] `structure/beat-sheet.md` has opening, midpoint, ending
- [ ] `screenplay/selected-scenes/opening.fountain` has a playable scene

You now have a working story system.
